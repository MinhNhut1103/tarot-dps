...
const translations = {
  ...
  // We'll map the minor arcana similarly with simplified short vietnamese concepts
  "Ace of Cups": {
...
Object.keys(translations).forEach(englishName => {
  // Regex finding the object blocks with string templates matching `name: "${englishName}"`
  // We'll replace using regex to modify 'name' and 'description' specifically
  
  const blockRegex = new RegExp(`name:\\s*['"\`]${englishName}['"\`]\\s*,\\s*description:\\s*(['"\`])([\\s\\S]*?)\\1`, 'g');
  
  outputContent = outputContent.replace(blockRegex, (match, quote) => {
    // Escape descriptions if we use backticks or quotes
    const safeDesc = translations[englishName].description.replace(/\n/g, '\\n').replace(/"/g, '\\"');
    return `name: "${translations[englishName].name}",\n    description:\n      "${safeDesc}"`;
  });
});
